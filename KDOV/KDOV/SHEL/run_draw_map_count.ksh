...
file_list1=($(find ${OUT_DIR1}/ -type f -name "OB4DA_MAP_${SDATE}_${EDATE}_COUNT*" | sort -n))
file_list2=($(find ${OUT_DIR2}/ -type f -name "OB4DA_MAP_${SDATE}_${EDATE}_COUNT*" | sort -n))
file_num=${#file_list1[@]}
...
#------------------------------------------------------------------------------------
  ...
  if [[ ${ilist} == *"grndgnss"* ]] || [[ ${ilist} == *"scatwind"* ]] || [[ ${ilist} == *"surface"* ]]; then
    ksh $DRAW_DIR/draw_MAP_sfc_count.sh        $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist 
    ksh $DRAW_DIR/draw_MAP_sfc_count_diff.sh        $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist 
  echo "-"
  elif [[ ${ilist} == *"amv"* ]] || [[ ${ilist} == *"gpsro"* ]] || [[ ${ilist} == *"sonde"* ]] || [[ ${ilist} == *"aircraft"* ]]; then
    ksh $DRAW_DIR/draw_MAP_prs_count.sh       $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist 
    ksh $DRAW_DIR/draw_MAP_prs_count_diff.sh       $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist 
  echo "-"
  else
    ksh $DRAW_DIR/draw_MAP_chn_count.sh        $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist "${sel_chn}" "${chn_prs}" 
    ksh $DRAW_DIR/draw_MAP_chn_count_diff.sh        $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $CTL_NAME $EXP_NAME $ilist "${sel_chn}" "${chn_prs}" 
   echo "-"
   fi 
