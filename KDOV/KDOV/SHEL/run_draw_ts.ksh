...
#------------------------------------------------------------------------------------
   ...
   if [[ ${ilist} == *"scatwind"* ]] || [[ ${ilist} == *"surface"* ]]; then
    ksh $DRAW_DIR/draw_TS_sfc.sh  $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $SET_OBS11 $SET_OBS22 $ilist ${CTLNAME} ${EXPNAME} ${INTV}
   echo ""
   elif [[ ${ilist} == *"amv"* ]] || [[ ${ilist} == *"gpsro"* ]] || [[ ${ilist} == *"sonde"* ]] || [[ ${ilist} == *"aircraft"* ]]; then
    ksh $DRAW_DIR/draw_TS_prs.sh   $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $SET_OBS11 $SET_OBS22 $ilist ${CTLNAME} ${EXPNAME} ${INTV}
   echo ""
   else
    ksh $DRAW_DIR/draw_TS_chn.sh   $HOME $SDATE $EDATE $FIG_DIR $OUT_DIR $SET_OBS11 $SET_OBS22 $ilist "${sel_chn}" ${CTLNAME} ${EXPNAME} ${INTV}
    echo ""
   fi 
