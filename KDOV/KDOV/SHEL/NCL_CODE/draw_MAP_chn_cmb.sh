...
outfig  = "${FIGDIR}/MAP/OB4DA_MAP_${mdl_name[$nmdl]}_${list}_ch"+ch_num+"_"+prs_nnum+"hPa_${SDATE}_${EDATE}_${var_name[$nvar]}"
...
;=================================
  ...
  if ("${nvar}" .eq. "0") then
   if ("${list}" .eq. "atms" .or. "${list}" .eq. "iasi" .or. "${list}" .eq. "amsua") then
  ...
  ;;res@mpLandFillColor  = "white"     ; choose color of continents.
                                            ; must be in colormap  
  res@tiMainString = "${list1} ch"+ch_num+"     ${mdl[$nmdl]}"
...
;----- TEXT ---------------------------------------------------
 ...
 if ("${nvar}" .eq. "0") then
  if ("${list}" .eq. "atms" .or. "${list}" .eq. "iasi" .or. "${list}" .eq. "amsua") then
   ...
   ndc_int  = 0.1365                                                             
   ndc_yst  = 0.28                                                               
   gsn_text_ndc(wks, "-1.0", ndc_xst,                 ndc_yst, txres)              
   gsn_text_ndc(wks, "-0.8", ndc_xst+(ndc_int*0.5),   ndc_yst, txres)              
   gsn_text_ndc(wks, "-0.6", ndc_xst+(ndc_int*1.0),   ndc_yst, txres)              
   gsn_text_ndc(wks, "-0.4", ndc_xst+(ndc_int*1.5),   ndc_yst, txres)              
   gsn_text_ndc(wks, "-0.2", ndc_xst+(ndc_int*2.0),   ndc_yst, txres)              
   gsn_text_ndc(wks, "0",  ndc_xst1+(ndc_int*2.5),  ndc_yst, txres)              
   gsn_text_ndc(wks, "0.2",  ndc_xst1+(ndc_int*3.0),  ndc_yst, txres)              
   gsn_text_ndc(wks, "0.4",  ndc_xst1+(ndc_int*3.5),  ndc_yst, txres)              
   gsn_text_ndc(wks, "0.6",  ndc_xst1+(ndc_int*4.0),  ndc_yst, txres)              
   gsn_text_ndc(wks, "0.8",  ndc_xst1+(ndc_int*4.5),  ndc_yst, txres)              
   gsn_text_ndc(wks, "1.0",  ndc_xst1+(ndc_int*5.0),  ndc_yst, txres) 
  else
   ndc_xst  = 0.177                                                              
   ndc_xst1 = 0.18                                                               
   ndc_int  = 0.1365                                                             
