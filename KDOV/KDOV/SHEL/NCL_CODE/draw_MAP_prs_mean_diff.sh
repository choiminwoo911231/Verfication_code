...
if ("${list}" .eq. "gpsro") then
 outfig  = "${FIGDIR}/MAP/OB4DA_MAP_${list}_"+"BA"+"_${prs_idx[$nprs]}hPa_${SDATE}_${EDATE}_${var_name[$nvar]}"
else
 outfig  = "${FIGDIR}/MAP/OB4DA_MAP_${list}_${var_idx[$nvdx]}_${prs_idx[$nprs]}hPa_${SDATE}_${EDATE}_${var_name[$nvar]}"
end if
print("outfig :"+outfig)
 ...
 ctl1 = ctl0->${idx}(:,${nprs},${nvdx},:,0:359)
 exp1 = exp0->${idx}(:,${nprs},${nvdx},:,0:359)
 ctl1@_FillValue = -999.
 ...
 ctl_std = dim_avg_n_Wrap(ctl1,0)
 exp_std = dim_avg_n_Wrap(exp1,0)
...
;   res@cnLevelSpacingF  =  0.05 
  ...
  if ("${list1}" .eq. "GPSRO") then                                   
   res@tiMainString  = "${list1} (10~S~4~N~)     ${EXP_NAME}-${CTL_NAME}"   ; add center string  
  end if 
  res@gsnLeftString  = "${var_name1[$nvar]}"   ; add center string
...
;----- HISTOGRAM ---------------------------------------------------
  ...
  xtick_st  = res@cnMinLevelValF-0.5
...
;----- TEXT ---------------------------------------------------
  ...
  gsn_text_ndc(wks, "-4", ndc_xst,                 ndc_yst, txres)
  gsn_text_ndc(wks, "-3", ndc_xst+(ndc_int*0.5),   ndc_yst, txres)
  gsn_text_ndc(wks, "-2", ndc_xst+(ndc_int*1.0),   ndc_yst, txres)
  gsn_text_ndc(wks, "-1", ndc_xst+(ndc_int*1.5),   ndc_yst, txres)
  gsn_text_ndc(wks, "0",  ndc_xst+(ndc_int*2.05),  ndc_yst, txres)
  gsn_text_ndc(wks, "1",  ndc_xst1+(ndc_int*2.5),  ndc_yst, txres)
  gsn_text_ndc(wks, "2",  ndc_xst1+(ndc_int*3.0),  ndc_yst, txres)
  gsn_text_ndc(wks, "3",  ndc_xst1+(ndc_int*3.5),  ndc_yst, txres)
  gsn_text_ndc(wks, "4",  ndc_xst1+(ndc_int*4.0),  ndc_yst, txres)
