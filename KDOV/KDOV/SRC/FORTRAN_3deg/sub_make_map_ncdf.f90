module sub_make_map_ncdf
  ...
  subroutine ncdf_ch_map(var, var1, bkg, nx, ny, nt, ch_info, obsname, mdlname, out_dir, sd, ed)
     ...
     real, dimension(:,:,:,:), intent(in) :: var, var1, bkg
     integer :: nch
     integer :: ncid1
     ...
     fname_out=trim(trim(out_dir)//trim(mdlname)//'/OB4DA_MAP_'//trim(sd)//'_'//trim(ed)//'_STAT_'//trim(obsname)//'.nc')
     ...
     call check(nf90_def_var(ncid1, "OmB",NF90_REAL, data_dimid_4d, varid1))
     call check(nf90_def_var(ncid1, "CmB",NF90_REAL, data_dimid_4d, varid2))
     call check(nf90_def_var(ncid1, "B",  NF90_REAL, data_dimid_4d, varid3))
     call check(nf90_put_var(ncid1, varid1,  var))
     call check(nf90_put_var(ncid1, varid2,  var1))
     call check(nf90_put_var(ncid1, varid3,  bkg))
  ...
  subroutine ncdf_nch_map(var, var1, bkg, nx, ny, nt, obsname, mdlname, out_dir, sd ,ed)
     character (len=256), intent(in) :: obsname, out_dir, sd, ed, mdlname
     character (len=256)             :: fname_out
     integer, intent(in)             :: nx, ny, nt
     real, dimension(:,:,:,:), intent(in) :: var, var1, bkg
  ...
  subroutine ncdf_prs_map(var, bkg, nx, ny, nt, obsname, mdlname, out_dir, sd ,ed, prs)
     character (len=256), intent(in) :: obsname, out_dir, sd, ed, mdlname
     character (len=256)             :: fname_out
     integer, intent(in)             :: nx, ny, nt
     real, dimension(:,:,:,:,:), intent(in) :: var, bkg
     ...
     call check(nf90_def_var(ncid1, "OmB",NF90_REAL, data_dimid_5d, varid1))
     call check(nf90_put_var(ncid1, varid1,  var))
     call check(nf90_def_var(ncid1, "B",NF90_REAL, data_dimid_5d, varid2))
     call check(nf90_put_var(ncid1, varid2,  bkg))
